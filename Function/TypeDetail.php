...
class TypeDetail{  
    ...
    public static function getTypeBySL($offset, $Count){
        $query = "SELECT * FROM `typedetail` WHERE 1 LIMIT $offset, $Count";
        $parameters = []; 
        $resultType = 2; 
        return DP::run_query($query, $parameters, $resultType);
    }
    public static function getTypeDetailByTypeId($TypeId){
        $query = "SELECT * FROM `typedetail` td WHERE td.TypeId=$TypeId";
        $parameters = []; 
        $resultType = 2; 
        return DP::run_query($query, $parameters, $resultType);
    }
